...
use Plugin\EccubePaymentLite42\Service\GmoEpsilonRequest\RequestCreateRegularOrderService;
...
use Plugin\EccubePaymentLite42\Service\IsExpireCreditCardService;
use Plugin\EccubePaymentLite42\Service\IsResumeRegularOrder;
use Plugin\EccubePaymentLite42\Service\Mail\OrderCreationBatchResultMailService;
use Plugin\EccubePaymentLite42\Service\Mail\RegularSpecifiedCountNotificationMailService;
use Plugin\EccubePaymentLite42\Service\Method\Credit;
use Plugin\EccubePaymentLite42\Service\Method\Reg_Credit;
use Plugin\EccubePaymentLite42\Service\NextDeliveryDateService;
use Plugin\EccubePaymentLite42\Service\RegularCreditService;
use Plugin\EccubePaymentLite42\Service\UpdateNormalPaymentOrderService;
use Plugin\EccubePaymentLite42\Service\UpdateRegularOrderService;
use Plugin\EccubePaymentLite42\Service\UpdateRegularStatusService;
...
class CreateRegularOrderController extends AbstractController
{
     ...
     *     name="eccube_payment_lite42_admin_regular_index_add",
     *     methods={"POST"}
     * )
     */
        ...
        try {
            ...
            if (empty($ids)) {
                ...
                return $this->redirectToRoute(
                    'eccube_payment_lite42_admin_regular_index_page', [
                    'page_no' => $page_no,
                ]);
            }
            /** @var Config $Config */
            ...
            if (!is_null($Order->getPayment()) &&
                ...
                if ($userInfoResults['status'] === 'OK') {
                    if ($this->isExpireCreditCardService->handle($userInfoResults['cardExpire'])) {
                        ...
                        logs('gmo_epsilon')->warning('定期ID: '.$RegularOrder->getId().'について、クレジットカードの有効期限が切れているため、受注の作成・イプシロン決済サービス決済の登録は行っていません。');
                        $this->entityManager->commit();

                        return $this->forwardRoute($ids);
                    }
                }
                $results = $this->requestCreateRegularOrderService->handle($RegularOrder, $Order, 'eccube_payment_lite42_admin_regular_index_add');
            ...
            logs('gmo_epsilon')->error($e->getMessage());
