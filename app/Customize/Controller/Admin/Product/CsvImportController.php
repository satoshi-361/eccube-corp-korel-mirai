...
class CsvImportController extends AbstractCsvImportController
{
            ...
            if ($form->isValid()) {
                ...
                if (!empty($formFile)) {
                    ...
                    foreach ($data as $row) {
                        ...
                        if (StringUtil::isNotBlank($row['一言'])) {
                            $Product->setOneWord($row['一言']);
                        }

                        if (StringUtil::isNotBlank($row['内容'])) {
                            $Product->setDescriptionDetail($row['内容']);
                        }

                        if (StringUtil::isNotBlank($row['カラー'])) {
                            $Product->setColor($row['カラー']);
                        }

                        if (StringUtil::isNotBlank($row['使用用途'])) {
                            $Product->setUsage0($row['使用用途']);
                        }

                        if (StringUtil::isNotBlank($row['キャッチコピー'])) {
                            $Product->setCatchCopy($row['キャッチコピー']);
                        }

                        if (StringUtil::isNotBlank($row['タイトル1'])) {
                            $Product->setTitle1($row['タイトル1']);
                        }

                        if (StringUtil::isNotBlank($row['詳細1'])) {
                            $Product->setDetail1($row['詳細1']);
                        }

                        if (StringUtil::isNotBlank($row['タイトル2'])) {
                            $Product->setTitle2($row['タイトル2']);
                        }

                        if (StringUtil::isNotBlank($row['詳細2'])) {
                            $Product->setDetail2($row['詳細2']);
                        }

                        if (StringUtil::isNotBlank($row['タイトル3'])) {
                            $Product->setTitle3($row['タイトル3']);
                        }

                        if (StringUtil::isNotBlank($row['詳細3'])) {
                            $Product->setDetail3($row['詳細3']);
                        }

                        if (StringUtil::isNotBlank($row['タイトル4'])) {
                            $Product->setTitle4($row['タイトル4']);
                        }

                        if (StringUtil::isNotBlank($row['詳細4'])) {
                            $Product->setDetail4($row['詳細4']);
                        }

                        if (StringUtil::isNotBlank($row['_商品名'])) {
                            $Product->setPropertyName($row['_商品名']);
                        }

                        if (StringUtil::isNotBlank($row['_カラー'])) {
                            $Product->setPropertyColor($row['_カラー']);
                        }

                        if (StringUtil::isNotBlank($row['サイズ'])) {
                            $Product->setPropertySize($row['サイズ']);
                        }

                        if (StringUtil::isNotBlank($row['材質'])) {
                            $Product->setPropertyMaterial($row['材質']);
                        }

                        if (StringUtil::isNotBlank($row['生産国'])) {
                            $Product->setPropertyCountry($row['生産国']);
                        }

                        if (StringUtil::isNotBlank($row['用途'])) {
                            $Product->setPropertyUsage($row['用途']);
                        }

                        if (StringUtil::isNotBlank($row['特許番号'])) {
                            $Product->setPropertyJan($row['特許番号']);
                        }

                        if (StringUtil::isNotBlank($row['注意事項'])) {
                            $Product->setPropertyNote($row['注意事項']);
                        }

                        if (StringUtil::isNotBlank($row['既存サイト'])) {
                            $Product->setExistingSite($row['既存サイト']);
                        }
    ...
    {
        if ( !isset($row['カテゴリー小']) && !isset($row['カテゴリー大']) ) {
            return;
        }
        ...
        if (StringUtil::isNotBlank($category = $row['カテゴリー小'])) {
            // カテゴリの登録
            $sortNo = 1;
            $categoriesIdList = [];
            $line = $data->key() + 1;
        ...
        if (StringUtil::isNotBlank($category = $row['カテゴリー大'])) {
            // カテゴリの登録
            $sortNo = 1;
            $categoriesIdList = [];
            $line = $data->key() + 1;
    ...
    protected function createProductClass($row, Product $Product, $data, $headerByKey, $ClassCategory1 = null, $ClassCategory2 = null)
    {
        ...
        if (isset($row['品番'])) {
            if (StringUtil::isNotBlank($row['品番'])) {
                $ProductClass->setCode(StringUtil::trimAll($row['品番']));
            } else {
        ...
        if (isset($row['金額']) && StringUtil::isNotBlank($row['金額'])) {
            preg_match('/\d+/', $row['金額'], $matches);
            $price02 = implode('', $matches);
            $errors = $this->validator->validate($price02, new GreaterThanOrEqual(['value' => 0]));
            if ($errors->count() === 0) {
                $ProductClass->setPrice02($price02);
            } else {
                $message = trans('admin.common.csv_invalid_greater_than_zero', ['%line%' => $line, '%name%' => '金額']);
                $this->addErrors($message);
            }
        } else {
            $ProductClass->setPrice02(0);
            // $message = trans('admin.common.csv_invalid_required', ['%line%' => $line, '%name%' => '金額']);
            // $this->addErrors($message);
        }
