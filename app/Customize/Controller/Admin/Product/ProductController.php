...
class ProductController extends AbstractController
{
    ...
    public function edit(Request $request, RouterInterface $router, CacheUtil $cacheUtil, $id = null)
    {
        ...
        if ('POST' === $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                // 商品詳細の設定
                foreach ( $Product->getProductDetails() as $index => $ProductDetail ) {
                    $ProductDetail->setSortNo($index);
                    $ProductDetail->setProduct($Product);

                    $this->entityManager->persist($ProductDetail);
                }

                // $inputDetails = $form->get('product_details')->getData();
                // $ProductDetails = $Product->getProductDetails();
                // $existedCount = $ProductDetails->count();

                // foreach ($inputDetails as $index => $detail) {
                //     if ( $index < $existedCount ) {
                //         // 商品詳細の更新
                //         $ProductDetail =  $ProductDetails->get($index);
                //     } else {
                //         // 商品詳細の追加
                //         $ProductDetail = new \Customize\Entity\ProductDetail();
    ...
    /**
     * @Route("/%eccube_admin_route%/product/detail/upload", name="admin_product_detail_upload", methods={"POST"})
     */
